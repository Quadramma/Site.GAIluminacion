...
currImg=img,i=0,grey;ctx.drawImage(img,0,0,width,height);var imageData=ctx.getImageData(0,0,width,height),px=imageData.data,length=px.length;if(BnWWorker)imagesArray.push({imageData:imageData,ctx:ctx,img:img});else{for(;i<length;i+=4){var k=px[i]*0.3+px[i+1]*0.59+px[i+2]*0.11;px[i]=~~(k*intensity+px[i]*(1-intensity));px[i+1]=~~(k*intensity+px[i+1]*(1-intensity));px[i+2]=~~(k*intensity+px[i+2]*(1-intensity))}ctx.putImageData(imageData,0,0);_onImageReady(img)}};var _injectTags=function($img,$imageWrapper){var pic=
...
0.3,10));newScrollOffset=Math.max(container[0][config.scrollOffset]-theStep,0)}else newScrollOffset=Math.max(container[0][config.scrollOffset]-config.step,0);container[0][config.scrollOffset]=newScrollOffset;if(newScrollOffset===0){clearInterval(scrollingId);if(config.autoPlay)nextScrollId=setTimeout(forward,config.delay*1E3)}};forward=function(){clearInterval(scrollingId);scrollingId=setInterval(scrollForward,config.speed)};forwardHover=function(){config.autoPlay=true;paused=false;clearInterval(scrollingId);
scrollingId=setInterval(scrollForward,config.speed)};pauseHover=function(){paused=true};backward=function(){clearInterval(scrollingId);scrollingId=setInterval(scrollBackward,config.speed)};resetClock=function(delay){config.delay=delay||config.delay;clearTimeout(nextScrollId);if(config.autoPlay)nextScrollId=setTimeout(forward,config.delay*1E3)};if(config.autoPlay)nextScrollId=setTimeout(forward,config.startDelay*1E3);container.bind("resetClock",function(delay){resetClock(delay)});container.bind("forward",
function(){clearTimeout(nextScrollId);forward()});container.bind("pauseHover",function(){pauseHover()});container.bind("forwardHover",function(){forwardHover()});container.bind("backward",function(){clearTimeout(nextScrollId);backward()});container.bind("speedUp",function(speed){if(typeof speed==="undefined")speed=Math.max(1,parseInt(config.speed/2,10));config.speed=speed});container.bind("speedDown",function(speed){if(typeof speed==="undefined")speed=config.speed*2;config.speed=speed})})}})(jQuery);
